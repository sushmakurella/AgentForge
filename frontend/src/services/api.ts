...
export async function getProviders(): Promise<ProviderMeta[]> {
  const res = await apiFetch(`${API_BASE}/agents/meta/providers`);
...
export async function getAgent(idOrSlug: string): Promise<Agent> {
  const res = await apiFetch(`${API_BASE}/agents/${idOrSlug}`);
...
export async function getOrganizationMembers() {
  const res = await apiFetch(`${API_BASE}/organization/members`);
...
export async function updateMemberRole(userId: string, role: string) {
  const res = await apiFetch(`${API_BASE}/organization/members/${userId}/role`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function getAgentPermissions(agentId: string) {
  const res = await apiFetch(`${API_BASE}/agents/${agentId}/permissions`);
